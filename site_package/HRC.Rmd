---
title: "Working with HRC imputed genotypes"
output:
  html_document:
    highlight: tango
    toc: true
    toc_float:
      collapsed: false
bibliography: BBMRIomics.bib
---

```{r, child="_setup.Rmd"}
```

## Find the vcf-files ##

```{r, vcffiles}
library(BBMRIomics)
dir(VM_BASE_DATA)
dir(VM_BASE_DATA, "HRC_Imputation")
vcfs <- dir(file.path(VM_BASE_DATA, "HRC_Imputation",
                      "CODAM"), pattern = "filtered.dose.vcf.gz$", full.names = TRUE,
            recursive = TRUE)
vcfs
```

## Inspect eQTL single SNP using `VariantAnnotation` ##

Example eQTL from Zhernakova et al. [@Zhernakova2017] (see Figure 2:
Identification of the strongest modifiers of eQTL effects).

```{r, annotation}
library(org.Hs.eg.db)
select(org.Hs.eg.db, keys="NOD2", keytype="SYMBOL", columns="ENSEMBL")
library(SNPlocs.Hsapiens.dbSNP142.GRCh37)
gr <- rsidsToGRanges("rs1981760")
```


<!-- Set "eval=FALSE" for chunk below, as execution of statement "zipped <- bgzip([file=]from, [dest=]to)" yields "Error:" (HD dd 2017-03-20 18:09). -->
```{r, readvcf}
# ```{r, readvcf, eval=FALSE}
seqlevels(gr) <- gsub("ch", "", seqlevels(gr))
genome(gr) <- "hg19"
from <- vcfs[grepl("chr16", vcfs)]
to <- tempfile()
library(VariantAnnotation)

# zipped <- bgzip(from, to)
zipped <- bgzip(file=from, dest=to)

fl <- indexTabix(zipped, format="vcf")
vcf <- readVcf(fl, genome="hg19", param=gr)
rs1981760 <- geno(vcf)$GT[1,]

```

```{r, linkhrcrna}
<<<<<<< HEAD
# ```{r, linkhrcrna, eval=FALSE}
data(rnaSeqData_ReadCounts_cleaned)
hrc <- intersect(names(rs1981760), counts$hrc_id)
counts <- counts[,counts$hrc_id %in% hrc]
mid <- match(counts$hrc_id, names(rs1981760))
=======
data(rnaSeqData_ReadCounts_CODAM_cleaned)
hrc <- intersect(names(rs1981760), counts$imputation_id)
counts <- counts[,counts$imputation_id %in% hrc]
mid <- match(counts$imputation_id, names(rs1981760))
>>>>>>> 9b12544409e1154a7edd4b3c2d41652c74e47a38
rs1981760 <- rs1981760[mid]
cnts <- assays(counts)$data
dim(cnts)
NOD2 <- cnts[rownames(cnts) == "ENSG00000167207", ]
boxplot(NOD2~rs1981760)
```

## Inspect Multiple SNPs using `getGenotypes` ##

<<<<<<< HEAD
<!-- Dd 2017-03-20 18:13 leads to ("Knit"[-ting]) error: -->
<!-- -- -->
<!-- "Error in counts$hrc_id : object of type 'closure' is not subsettable -->
<!-- Calls: <Anonymous> ... withVisible -> eval -> eval -> getGenotypes -> .getHRC -> match -->
<!-- Execution halted" -->
<!-- -- -->
<!-- ==> set "eval=FALSE" for this chunk (HD). -->
=======
> Replace this with the `MatrixeQTL` example from Rick/Joost?

>>>>>>> 9b12544409e1154a7edd4b3c2d41652c74e47a38
```{r, getgenotypes}
# ```{r, getgenotypes, eval=FALSE}
library(SNPlocs.Hsapiens.dbSNP142.GRCh37)
gr <- rsidsToGRanges(c("rs9302752", "rs1981760"))
data(rnaSeqData_ReadCounts_cleaned)
<<<<<<< HEAD

dosages <- BBMRIomics::getGenotypes(imputation_id = counts$hrc_id, biobank="ALL", snps=gr, type="HRC", geno="DS", BASE=VM_BASE_DATA)

=======
dosages <- getGenotypes(imputation_id = counts$imputation_id, biobank="ALL", snps=gr, type="HRC", geno="DS", BASE=VM_BASE_DATA)
>>>>>>> 9b12544409e1154a7edd4b3c2d41652c74e47a38
dosages[1:2, 1:5]
```

```{r, linkhrcrna2}
# ```{r, linkhrcrna2, eval=FALSE}
dim(counts)
dim(dosages)
head(counts$imputation_id)
head(colnames(dosages))
cnts <- assays(counts)$data
library(edgeR)

library(limma)
fit <- lmFit(cnts, dosages[1,]) ##run for 1 SNP
tstat <- fit$coef/fit$stdev.unscaled/fit$sigma
pval <- 2 * pnorm(-abs(tstat[,1]))
head(sort(pval, decreasing=FALSE))
```


## More information ##

see [VariantAnnotation](http://bioconductor.org/packages/VariantAnnotation/).

For example, extracting SNPs overlapping transcripts:

```{r, overlapsnps, eval=FALSE}
library(TxDb.Hsapiens.Ensembl.v71)
txdb <- TxDb.Hsapiens.Ensembl.v71
vcf <- readVcf(vcfs[15], genome="hg19")
rd <- rowRanges(vcf)
loc <- locateVariants(rd, txdb, CodingVariants())
head(loc, 6)
```

## References ##


<!-- END OF FILE. -->
