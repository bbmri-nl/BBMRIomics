...
preprocess <- function(sumExp) {
    ...
    message("[preprocess] drop features Zhou probes")
...
```{r}
phenotyped <- phenotyping(phenotype, features, methods = "lasso", ntop=5, verbose=TRUE)
...
```{r}
top <- phenotyped$top
y <- features[rownames(features) %in% names(top),]
colnames(y) <- NULL
...
validation <- validate(phenotype, features, methods = c("ridge", "elastic-net", "lasso", "gradient-boosting", "partial-least-squares"),
                       ntop = c(100, 250, 500, 1000, 2500, 5000), nrep = 5, verbose=FALSE)
