---
title: "GRCh38 RNA Freeze2 GoNL datasets"
output: 
  html_document:
    highlight: tango
    toc: true
...
Now we can use the Ensembl v91 annoation like any other `TxDb`-package.
...
gonl$ascertainment_criterion <- gsub("^Complete genomics sequencing$", "CG_subject", gonl$ascertainment_criterion)
gonl$ascertainment_criterion <- gsub("^GoNL / Complete genomics sequencing", "GoNL/CG_subject", gonl$ascertainment_criterion)
...
gonl$dna_quantificationmethod <- gsub("spectrofotometer|Spectofotometer|spectofotometer", "Spectrofotometer", gonl$dna_quantificationmethod)
table(gonl$dna_quantificationmethod)

## dna_extraction_method
gonl$dna_extraction_method <- gsub("salting out|Saltingout", "Salting out", gonl$dna_extraction_method)
gonl$dna_extraction_method <- gsub("QIAamp DNA minikit", "DNA Mini Kit (Qiaamp)", gonl$dna_extraction_method)
...
##select runs
files <- files[gsub("\\.tsv$", "", basename(files)) %in% gonl$run_id]
...
counts <- counts[grep("^Unassigned_", rownames(counts), invert=T),]
...
save(counts, file = file.path(VM_BASE_DATA, "RNASeq/v2.1.3/gene_read/", 
                              "rnaSeqData_ReadCounts_BIOS_Freeze2_GoNL_GRCh38.RData"),
     compress = "xz")
```
